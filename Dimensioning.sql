...
-- Step-2 Missing Data (How to handle NA)--7 rows are available
select 
    hour(index_record_ts) as Eachhours,
    * 
from 
    clean_aqi_dt 
where 
...
--Ensuring each records for evry hour are having 7 rows of data for Pollutant which shld be drilled to one row.
select 
        index_record_ts,
        country,
        state,
...
where 
        country = 'India' and
        state = 'Karnataka' and 
        station = 'BTM Layout, Bengaluru - CPCB' and --7 rows are retrieved
        index_record_ts = '2024-03-01 11:00:00.000'

-- step-3  transpose the data from rows to columns.
...
select 
    ...
    where 
        country = 'India' and
        state = 'Karnataka' and 
        station = 'BTM Layout, Bengaluru - CPCB' and 
        index_record_ts = '2024-03-01 11:00:00.000'
...
--Nearly 4.3K records are having NA values for pollutantID which should be replaced
select 
    hour(index_record_ts) as Eachhours,
    * 
from 
...
where 
    ...
    --creating an internal table for storing the state codes for the states in the dataset
    CREATE OR REPLACE TABLE Dev_DB.TRANSFORM_SCH.state_codes (
    state_name STRING,
    state_code STRING
);
...
select distinct city from clean_aqi_dt

--ensuring codes are available for all states
    select distinct sc.state_name,aqi.state
     from clean_aqi_dt  aqi
    inner join state_codes sc on sc.state_name=aqi.state
...
select 
        ...
        END as O3_AVG,
    from clean_aqi_dt aqi
    inner join state_codes sc ON aqi.state=sc.state_name
...
with step01_combine_pollutant_cte as (
    ...
    inner join state_codes sc ON aqi.state=sc.state_name
    group by 
        index_record_ts, country, state, city, station, latitude, longitude,sc.state_code
        order by country, city, station,sc.state_code
),
